...
export function ChatBot({ configId }: { configId: string }) {
  ...
  const iframeParams: IFrameParams = {
    // pageUrl: window.location.href,
  }

  // When the page loads, check to see if there is a conversationId in session storage
  // If there is, pass it as a URL param to the iframe
  ...
  useEffect(() => {
    if (runtime.name === "browser") {
      ...
      if (conversationId) {
        iframeParams.conversationId = conversationId;
      }
      iframeParams.pageUrl = pageUrl;
      iframeParams.referrerPageUrl = referrerPageUrl;
    }
  }, []);
  ...
  const iFrameUrl = new URL(`https://yext-chatbot-prod.up.railway.app/bots/${configId}`);
  const params = new URLSearchParams(JSON.stringify(iframeParams));
  ...
  return (
    <div className={cx(
      ...
      <Transition className="fixed right-4 bottom-24 lg:bottom-40 lg:right-10 w-80 lg:w-96 h-3/4 lg:h-2/3 xl:1/2  bg-white rounded-xl shadow-xl overflow-hidden" as="div" enter="transition-opacity duration-300" enterFrom="opacity-0" enterTo="opacity-100" leave="transition-opacity duration-300" leaveFrom="opacity-100" leaveTo="opacity-0" show={showChat}>
        <iframe
          ref={iframeRef}
          className="w-full h-full" src={`https://yext-chatbot-prod.up.railway.app/bots/${configId}`} />
      </Transition>
