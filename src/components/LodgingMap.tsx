...
import { Result, useSearchActions, useSearchState, Matcher } from "@yext/search-headless-react";
...
const LodgingMap = (): JSX.Element => {
  ...
  const results = useSearchState(s => s.vertical.results) as unknown as Result<Location>[] | undefined;
  const [showMore, setShowMore] = useState(false);

  const abbreviatedResults = showMore ? results : results?.slice(0, 8) ?? [];

  return (
    <div className="w-full flex flex-col lg:flex-row gap-x-4 h-full">
      <div className="lg:w-3/5 h-full">
        ...
        <div className="flex flex-col gap-y-2 mt-4">
          ...
          <div className="grid  grid-cols-1 lg:grid-cols-2 gap-y-1 gap-x-1">
            {
              abbreviatedResults?.map((location, index) => {
                ...
                return (
                  ...
                  >
                    <Link
                      ...
                      {`${index + 1}. ${location.rawData.name}`}
                    </Link>
                    <div>
                      <LocationLink className="text-xs" address={address} />
                    </div>
                  </div>
                )
              }
          ...
          <button
            className="text-green-1000 text-sm invisible lg:visible"
      ...
      <div className="lg:h-2/3 lg:w-2/5 shrink-0 relative rounded-lg overflow-hidden">
        <MapboxMap<Location>
          PinComponent={MapPin}
          mapboxOptions={{
